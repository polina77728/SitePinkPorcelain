...
//если сессии нет, то пользователь выбрасывает на кабинет залогиниться
if (!$_SESSION){
...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_post'])){
    ...
    if($title === '' || $content === '' || $topic === ''){
        ...
        array_push($errMsg, "Название статьи должно быть более 5-х символов");
    }else{
            $post = [
                'id_user' => $_SESSION['id'],
                'title' => $title,
...
if($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['id'])){
    $post = selectOne('posts', ['id' => $_GET['id']]);   // получить запись
